...
The VDC is clocked separately from the system clock in the C128. To have correct video sync timing, the VDC clock in the core has to run asynchronously from the master clock of the core. This causes noise on some video filter settings as the filters are clocked differently from the pixels generated by the VDC. If you experience noise on the 80 column display, change or disable the video filter.
